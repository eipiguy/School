function [ tp,yp ] = rk4ODEsysArrow( dydt,tspan,y0,h,varargin )
...
for i=1:m
    ...
    % find the minimum and maximunm values for each solution
    ymin = min(yp(i));
    ymax = max(yp(i));
    
    % create a grid of all the locations to place arrows
    [ meshX, meshY ] = meshgrid( ti:((tf-ti)/10):tf, ymin:((ymax-ymin)/10):ymax );
    
    % computing the matching grid of directional components
    % for the arrows takes some work:
    
    % take the solution and create a mesh that varies 
    % only the curent component
    for k=1:(m-1)
        if k=i-1
            yC = ymin:((ymax-ymin)/10):ymax;
            ym = cat(2,yp(:,k),yC(:,k+1));
    end
    ...
    subplot(sub(1+(i-1)*m)),quiver(meshX,meshY,meshX,dydt(meshX,meshY));
