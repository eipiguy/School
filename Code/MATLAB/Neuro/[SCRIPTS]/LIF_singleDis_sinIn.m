...
%--------------------------------------------------------------------------
aIn = 50;  % amplitude of current into the membrane (mA/cm^2)
fIn = 30;   % current frequency into the membrane (rad/s)
pIn = 0;    % phase of current into membrane (rad)
%--------------------------------------------------------------------------
In =@(t) sinIn(t,aIn,fIn,pIn);  % input control current (mA/cm2), 
                                % dependent on time
%--------------------------------------------------------------------------
...
firingPlotFileName = sprintf(...
    'firingPlot-singleDisconnectedLIF-sinInput_a%d,f%d,p%d.jpg',...
    aIn,fIn,pIn);
phasePlotFileName = sprintf(...
    'phasePlot-singleDisconnectedLIF-sinInput_a%d,f%d,p%d.avi',...
    aIn,fIn,pIn);
