function [ frames ] = phasePlot( tSol, ySol, dydt, varargin )
...
for i=1:m
    ...
    plot(tSol,ySol(:,i));       % Draw the solution vs time
    xLab = xlabel('t');         % Label the x-axis as the time axis
    yLab = ylabel(sprintf('y(%d)',i)); % Label the y-axis as the solution value
...
for i=1:m-1
    for j=i+1:m
        ...
        xlabel(sprintf('y(%d)',j)); % Label the x-axis as the greater dim
        ylabel(sprintf('y(%d)',i)); % Label the y-axis as the lesser dim
        set(gca,'FontSize',4+(8/m));
        xLab.FontSize = 4+(8/m);
        yLab.FontSize = 4+(8/m);
