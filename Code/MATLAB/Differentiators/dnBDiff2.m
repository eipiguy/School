function df = dnBDiff2( n,f,x,h,varargin )
% dnBDiff2: 
%   df = dnBDiff2( n,f,x,h,varargin )
% input:
%   n = the order of the derivative approximation
%   f = function to differentiate
%   x = x value to evaluate the derivative at
%   h = step size for approximations
...
c = zeros(n+2,1);
for i=0:n
    c(n+2-i) = ((-1)^(n+i))*((factorial(n+1)*nchoosek(n,i)) +nchoosek(n+1,i));
end
c(1) = ((-1)^(n+1+i));
disp(xspan);
disp(fspan);
